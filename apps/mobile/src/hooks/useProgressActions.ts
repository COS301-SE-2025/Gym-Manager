...
export function useProgressActions(classId: number) {
  ...
  const submitPartial = async (reps: number) => {
    const token = await getToken();
    await axios.post(
      `${config.BASE_URL}/live/live/${classId}/partial`,
      { reps },
      { headers: { Authorization: `Bearer ${token}` } }
    );
...
export async function coachStart(classId: number) {
  const token = await getToken();
  await axios.post(`${config.BASE_URL}/live/coach/live/${classId}/start`, {}, {
    headers: { Authorization: `Bearer ${token}` }
  });
}
...
export async function coachStop(classId: number) {
  const token = await getToken();
  await axios.post(`${config.BASE_URL}/live/coach/live/${classId}/stop`, {}, {
    headers: { Authorization: `Bearer ${token}` }
  });
}
