...
function useIntervalTicker(session: any | null) {
  ...
  let acc = 0, idx = timed[0].i;
  for (const t of timed) {
    if (elapsedRaw < acc + t.d) { idx = t.i; break; }
    acc += t.d;
  }
    ...
    <SafeAreaView style={s.root}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 26 }}>
        ...
        <TouchableOpacity
          style={[
            s.finishBtn,
            (!canSubmit || !allFilled || !allSubmitted || !noErrors || finishing) && s.finishBtnDisabled,
          ]}
...
const s = StyleSheet.create({
  ...
  notice:{ backgroundColor:'#1f1f1f', paddingHorizontal:10, paddingVertical:6, borderRadius:8, marginTop:8 },
  noticeText:{ color:'#bbb', fontWeight:'700' },

  endedBanner:{ backgroundColor:'#2a2a2a', padding:10, borderRadius:10, alignItems:'center', marginTop:8 },
  ...
  finishBtn:{ backgroundColor:'#d8ff3e', paddingVertical:14, borderRadius:12, alignItems:'center' },
  finishBtnDisabled:{ opacity:0.5 },
  finishBtnText:{ color:'#111', fontWeight:'900' },

  pausedOverlay:{ ...StyleSheet.absoluteFillObject, alignItems:'center', justifyContent:'center', zIndex:30 },
