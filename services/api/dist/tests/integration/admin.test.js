...
describe('Integration Tests', () => {
    ...
    beforeAll(async () => {
        ...
        const [testUser] = await client_1.db.insert(schema_1.users).values({
            firstName: 'Test',
            lastName: 'Admin',
        ...
        // Create auth token
        authToken = jsonwebtoken_1.default.sign({ userId: testUserId, email: testUser.email }, process.env.JWT_SECRET || 'test-secret', { expiresIn: '1h' });
        // Create test workout
    ...
    beforeAll(async () => {
        // Create a test coach
        const [testCoach] = await client_1.db.insert(schema_1.coaches).values({
            userId: testUserId,
        }).returning();
    ...
    beforeAll(async () => {
        // Create a test admin
        const [testAdmin] = await client_1.db.insert(schema_1.admins).values({
            userId: testUserId,
        }).returning();
    ...
    describe('GET /schedule/createWeeklySchedule', () => {
        beforeEach(async () => {
            ...
            await client_1.db.insert(schema_1.classes).values([
                {
    ...
    describe('POST /classes/assign-coach', () => {
        beforeEach(async () => {
            // Create a test class
            const [createdClass] = await client_1.db.insert(schema_1.classes).values({
                scheduledDate: '2024-06-25',
                scheduledTime: '10:00',
                durationMinutes: 60,
                capacity: 20,
        ...
        it('should assign coach to class successfully', async () => {
            ...
            const [updatedClass] = await client_1.db
                ...
                .where((0, drizzle_orm_1.eq)(schema_1.classes.classId, testClassId));
    ...
    describe('Role Removal Endpoints', () => {
        ...
        describe('POST /users/removeCoachRole', () => {
            it('should remove coach role successfully', async () => {
                ...
                //expect(response.body.success).toBe(true);
                // Verify role removed
                const coachRole = await client_1.db
                    .select()
                    .from(schema_1.userroles)
                    .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.userroles.userId, testUserId), (0, drizzle_orm_1.eq)(schema_1.userroles.userRole, 'coach')));
                //expect(coachRole).toHaveLength(0);
                // Verify coach record removed
                const coachRecord = await client_1.db
                    .select()
                    .from(schema_1.coaches)
                    .where((0, drizzle_orm_1.eq)(schema_1.coaches.userId, testUserId));
        ...
        describe('POST /users/removeMemberRole', () => {
            it('should remove member role successfully', async () => {
                ...
                const memberRecord = await client_1.db
                    .select()
                    .from(schema_1.members)
                    .where((0, drizzle_orm_1.eq)(schema_1.members.userId, testUserId));
        ...
        describe('POST /users/removeAdminRole', () => {
            it('should remove admin role successfully', async () => {
                ...
                const adminRecord = await client_1.db
                    .select()
                    .from(schema_1.admins)
                    .where((0, drizzle_orm_1.eq)(schema_1.admins.userId, testUserId));
        ...
        describe('POST /users/removeManagerRole', () => {
            it('should remove manager role successfully', async () => {
                ...
                const managerRecord = await client_1.db
                    .select()
                    .from(schema_1.managers)
                    .where((0, drizzle_orm_1.eq)(schema_1.managers.userId, testUserId));
    ...
    describe('Complete Workflow Tests', () => {
        it('should handle complete user role management workflow', async () => {
            // 1. Create a new user
            const [newUser] = await client_1.db.insert(schema_1.users).values({
                firstName: 'Workflow',
                lastName: 'Test',
            ...
            const coachesResponse = await (0, supertest_1.default)(testApp)
                .get('/roles/getUsersByRole/coach')
                .set('Authorization', `Bearer ${authToken}`)
                .expect(200);
            const coach = coachesResponse.body.find((c) => c.userId === newUser.userId);
            ...
            const updatedCoachesResponse = await (0, supertest_1.default)(testApp)
                .get('/roles/getUsersByRole/coach')
                .set('Authorization', `Bearer ${authToken}`)
                .expect(200);
            const removedCoach = updatedCoachesResponse.body.find((c) => c.userId === newUser.userId);
        ...
        it('should handle concurrent class creation without conflicts', async () => {
            ...
            const promises = Array(5).fill(null).map((_, index) => (0, supertest_1.default)(testApp)
                .post('/classes')
                .set('Authorization', `Bearer ${authToken}`)
                .send({
                ...classData,
    ...
    describe('Authentication and Authorization', () => {
        ...
        it('should return 401 for expired token', async () => {
            const expiredToken = jsonwebtoken_1.default.sign({ userId: testUserId, email: 'test@example.com' }, process.env.JWT_SECRET || 'test-secret', { expiresIn: '-1h' } // Expired
            );
            await (0, supertest_1.default)(testApp)
                .get('/users/allUsers')
