 ...
 */
const ctrl = __importStar(require("../../controllers/ongoingClassController"));
...
// ──────────────────── Helpers ───────────────────────────────
const mockReq = (uid, roles = [], extras = {}) => ({ user: uid ? { userId: uid, roles } : undefined, ...extras });
const mockRes = () => {
    const r = {};
...
describe('getLiveClass', () => {
    ...
    it('coach branch returns participants', async () => {
        client_1.db.select
            ...
            .mockReturnValueOnce((0, builder_1.builder)([{ userId: 40 }, { userId: 41 }]));
...
describe('submitScore', () => {
    const baseReq = (userId, roles, body) => mockReq(userId, roles, { body });
    ...
    describe('coach flow', () => {
        it('403 if coach not assigned to class', async () => {
            client_1.db.select.mockReturnValue((0, builder_1.builder)([{ coachId: 999 }])); // wrong coach
        ...
        it('successfully upserts scores', async () => {
            client_1.db.select.mockReturnValue((0, builder_1.builder)([{ coachId: 30 }]));
            const req = baseReq(30, ['coach'], {
                classId: 8,
                scores: [
                    { userId: 2, score: 90 },
