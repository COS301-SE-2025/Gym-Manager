...
export class LiveClassController {
  ...
  submitScore = async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (
        ...
        return res
          .status(msg === 'CLASS_ID_REQUIRED' || msg === 'SCORE_REQUIRED' ? 400 : 403)
  ...
  getRealtimeLeaderboard = async (req: Request, res: Response) => {
    try {
      ...
      if (e.message === 'WORKOUT_NOT_FOUND_FOR_CLASS')
  ...
  postIntervalScore = async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const msg = e.message || '';
      const code =
        msg === 'INVALID_STEP_INDEX' ||
        msg === 'STEP_INDEX_OUT_OF_RANGE' ||
        msg === 'NOT_INTERVAL_WORKOUT'
          ? 400
          : msg === 'NOT_BOOKED'
            ? 403
  ...
  postEmomMark = async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const finishSeconds =
        req.body?.finishSeconds == null ? null : Math.max(0, Number(req.body.finishSeconds));
      ...
      await this.service.postEmomMark(classId, req.user.userId, {
        minuteIndex,
        finished,
        finishSeconds,
        exercisesCompleted,
      ...
      const msg = e.message || '';
      const code =
        msg === 'SESSION_NOT_FOUND' || msg === 'NOT_EMOM_WORKOUT'
          ? 400
          : msg === 'NOT_BOOKED'
            ? 403
            : 500;
  ...
  coachPostIntervalScore = async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const msg = e.message || '';
      const code = ['INVALID_STEP_INDEX', 'STEP_INDEX_OUT_OF_RANGE'].includes(msg)
        ? 400
        : msg === 'NOT_CLASS_COACH'
          ? 403
          : 500;
  ...
  coachPostEmomMark = async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      await this.service.coachPostEmomMark(
        classId,
        req.user!.userId,
        userId,
        minuteIndex,
  ...
  emomSetTotal = async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      await this.service.coachEmomSetTotalEndedOnly(
        classId,
        req.user!.userId,
        userId,
        totalSeconds,
